...
def runDrugNCF():
    ...
    elif const.CURRENT_DATA == 'AEOLUS':
        const.LAMB = 3e-5  # 5e-4
        const.LEARNING_RATE = 0.0075  # 0.0075
    elif const.CURRENT_DATA == 'SIDER':
        const.LAMB = 1e-5
        const.LEARNING_RATE = 0.001 # xinya
    elif const.CURRENT_DATA == 'TOX':
        const.LAMB = 1e-5
        const.LEARNING_RATE = 0.001 # xinya
    else:
    ...
    if options.save_dir:
        save_model(model, options) # xinya  
...
def save_model(model, options):
    ...
    file_name = f"model_{options.modelName}_data_{options.data}_embed_{options.embedding_size}_featuremode_{options.feature}"
    full_path = os.path.join(options.save_dir, file_name)
    ...
    try:
        ...
        if hasattr(model, 'state_dict'):
            ...
            print(f"model saved (PyTorch): {full_path}")
            return full_path
    except Exception as e:
        print(f"saveing failed(PyTorch): {e}")
    ...
    try:
        ...
        print(f"model saved (scikit-learn): {full_path}")
        return full_path
    except Exception as e:
        print(f"saveing failed (joblib): {e}")
    ...
    try:
        ...
        with open(full_path, 'wb') as f:
            pickle.dump(model, f)
        print(f"model saved (pickle): {full_path}")
        return full_path
    except Exception as e:
        print(f"saveing failed: {e}")
        return None  # xinya
...
if __name__ == "__main__":
    ...
    parser.add_option("-d", "--data", dest="data", type='string', default="AEOLUS", help="data: Liu, AEOLUS, SIDER, TOX [default: "
                                                                                         "%default]") # xinya
...
#     parser.add_option("-i", "--init", dest="init", action='store_true', default=False)
    ...
    parser.add_option("--save-dir", dest="save_dir", type='string', default="./final_models",
                  help="Directory to save the model [default: %default]") # xinya
...
#         DataFactory.genKFoldECFPAEOLUS()  # 生成K折AEOLUS数据
        DataFactory.genKFoldECFPSIDER()  # 生成K折SIDER数据 xinya
#         DataFactory.genKFoldECFPTOX() # 生成K折毒性数据 xinya
    ...
    if options.data == "Liu":
        ...
        num_DRUG = 1358
        num_ADR = 2707
    elif options.data == "SIDER":
        const.CURRENT_KFOLD = const.KFOLD_FOLDER_EC_SIDER
        const.CURRENT_DATA = "SIDER"
        # num_DRUG = 1193
        num_DRUG = 918
        num_ADR = 2082  # xinya  
    elif options.data == "TOX":
        const.CURRENT_KFOLD = const.KFOLD_FOLDER_EC_TOX
        const.CURRENT_DATA = "TOX"
        num_DRUG = 22399
        num_ADR = 27  # xinya    
    else:
        print("Fatal error: Unknown data. Only"
              " Liu, AEOLUS, SIDER and TOX datasets are supported.")
    ...
    if const.FEATURE_MODE == const.BIO2RDF_FEATURE:  # 设置field_dims
        field_dims = [6712]
        # field_dims = [200] # TOXRIC为200，替换BIO2RDF为RDKID
        # field_dims = [1024] # 替换了BIO2RDF为Morgan
...
#         field_dims = [1024]
        ...
        field_dims = [881, 6712]
#         field_dims = [2048, 6712]
#         field_dims = [167, 6712]
#         field_dims = [1024, 6712]
        # field_dims = [216, 6712]
        # field_dims = [1024, 200]
    # if modelName == "KNN":
    #    runKNN()
